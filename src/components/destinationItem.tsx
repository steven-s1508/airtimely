...
export const DestinationItem = React.memo(function DestinationItem({ item, isPinned, onTogglePin }: { item: DisplayableEntity; isPinned: boolean; onTogglePin: (entityId: string) => void }) {
			...
			<View style={[destinationItemStyles.container, containerStyle]}>
				<VStack style={destinationItemStyles.containerInner}>
					<HStack style={{ flexDirection: "row", alignItems: "center", gap: 8 }}>
						...
						{!isPinned ? (
							<Pressable android_ripple={{ color: colors.primaryTransparent, foreground: true }} onPress={handleTogglePin}>
								<View style={{ padding: 4, borderWidth: 1, borderColor: colors.primaryLight, borderRadius: 4, overflow: "hidden" }}>
									<Icon name="favorite" fill={colors.primaryLight} height={16} width={16} />
								</View>
							</Pressable>
						) : (
							<Pressable android_ripple={{ color: colors.primaryTransparentDark, foreground: true }} onPress={handleTogglePin}>
								<View style={{ padding: 4, borderWidth: 1, borderColor: colors.primaryLight, backgroundColor: colors.primaryLight, borderRadius: 4, overflow: "hidden" }}>
									<Icon name="favorite" fill={colors.primaryVeryDark} height={16} width={16} />
								</View>
							</Pressable>
						)}
					</HStack>
