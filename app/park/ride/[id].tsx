...
function AttractionItem({ name, waitTime, status, singleRiderWaitTime, hasVirtualQueue }: { name: string; waitTime: number; status: string; singleRiderWaitTime: number; hasVirtualQueue: boolean }) {
	const virtualQueue = (
	...
	return (
		<VStack style={{ gap: 16, padding: 8, borderWidth: 1, borderColor: colors.primary, backgroundColor: colors.primaryVeryDark, borderRadius: 6 }}>
			<HStack style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "flex-start", gap: 8 }}>
				...
				<Pressable android_ripple={{ color: colors.primaryTransparent, foreground: true }} onPress={() => console.log("Attraction statistics pressed")} style={{ flexDirection: "row", alignItems: "center", gap: 4, paddingHorizontal: 12, paddingVertical: 6, backgroundColor: colors.primaryVeryDark, borderWidth: 1, borderColor: colors.primaryLight, borderRadius: 4, overflow: "hidden" }}>
			...
			<SafeAreaView style={{ flex: 1 }}>
				<ScrollView>
					<VStack style={{ padding: 16, gap: 8 }}>
						<AttractionItem name="Space Mountain" status="Open" waitTime={45} />
						<AttractionItem name="Pirates of the Caribbean" status="Closed" waitTime={15} singleRiderWaitTime={5} />
						<AttractionItem name="Guardians of the Galaxy - Mission: BREAKOUT!" status="Open" waitTime={70} singleRiderWaitTime={55} />
						<AttractionItem name="Big Thunder Mountain Railroad" status="Open" waitTime={0} hasVirtualQueue />
						<AttractionItem name="It's a Small World" status="Open" waitTime={10} />
					</VStack>
				</ScrollView>
			</SafeAreaView>
		</View>
