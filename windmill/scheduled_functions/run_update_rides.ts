...
async function updateRidesTable(supabase: any) {
	...
	for (const park of parks) {
		...
		const { data: dbRides, error: dbRidesError } = await supabase.from("rides").select("id, external_id, name, is_active").eq("park_id", park.id);
		...
		for (const apiRide of apiRides) {
			...
			if (!dbRide) {
				...
				if (Object.keys(updates).length > 0) {
					const { error: updateError } = await supabase.from("rides").update(updates).eq("id", dbRide.id);
		...
		console.log(
			"DB external_ids:",
			dbRides.map((r: any) => r.external_id)
		);
		...
		for (const dbRide of dbRides) {
			if (dbRide.external_id && !apiRideIds.has(dbRide.external_id) && dbRide.is_active) {
				...
				const { error: deactivateError } = await supabase.from("rides").update({ is_active: false }).eq("id", dbRide.id);
