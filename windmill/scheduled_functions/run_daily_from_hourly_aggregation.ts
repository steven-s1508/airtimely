...
export async function main(date?: string, cleanup: boolean = true, forceUpdate: boolean = false) {
    ...
    for (const ride of rides) {
        try {
            ...
            const { data: existingDaily, error: dailyCheckError } = await supabase
            ...
            if (dailyCheckError && dailyCheckError.code !== 'PGRST116') { // PGRST116 = no rows found
            ...
            if (existingDaily && !forceUpdate) {
                ...
                const { count: hourlyCount, error: hourlyCountError } = await supabase
    ...
    if (cleanup && successCount > 0) {
        try {
            ...
            const { count, error: cleanupError } = await supabase
                ...
                .delete()
                .lt("recorded_at_local", cleanupDateStr);
